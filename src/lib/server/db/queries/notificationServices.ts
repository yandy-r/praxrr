...
export const notificationServicesQueries = {
  ...
  getById(id: string): NotificationService | undefined {
    return db.queryFirst<NotificationService>('SELECT * FROM notification_services WHERE id = ?', id);
  },
  ...
  update(id: string, input: UpdateNotificationServiceInput): boolean {
    ...
    const affected = db.execute(`UPDATE notification_services SET ${updates.join(', ')} WHERE id = ?`, ...params);
