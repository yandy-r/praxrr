...
Deno.test(
  'lidarr quality mapping: unmapped quality entries are excluded from Lidarr list results',
  async () => {
...
Deno.test(
  'lidarr quality mapping: mapped entries resolve correctly from lidarr_quality_definitions',
  async () => {
    const originalWarn = logger.warn;
...
Deno.test(
  'lidarr quality mapping: sonarr quality data does not leak into lidarr list results',
  async () => {
    const originalWarn = logger.warn;
    const originalInfo = logger.info;
