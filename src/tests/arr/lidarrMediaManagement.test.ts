...
`;
	...
	runTests(): void {
		...
		this.test(
			'[NM-03b] naming create lidarr name does not collide with sonarr name of same value',
			async () => {
				await this.bootstrapFixture();
		...
		this.test('[NM-04] naming edit lidarr rename persists and updates sync mapping reference', async () => {
			...
			assertEquals(
				namingConfigs.find((item) => item.arr_type === 'lidarr' && item.name === 'Lidarr-Naming-Renamed') !==
					undefined,
				true
			);
			assertEquals(
		...
		this.test('[NM-06] naming lidarr deep-link load returns Lidarr-native fields', async () => {
			...
			assertEquals(
				loaded.namingConfig.standard_track_format,
		...
		this.test('[QD-07] quality-definitions list returns only lidarr entries from lidarr_quality_definitions', async () => {
			await this.bootstrapFixture();
