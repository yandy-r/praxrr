...
const LIDARR_UNSUPPORTED_WORKFLOW_MESSAGE =
  "Lidarr does not support Rename and Upgrades in Profilarr yet.";
const LIDARR_SUPPORTED_WORKFLOW_MESSAGE =
  "You can still use Library and Releases.";

const CONNECTION_SUCCESS_MESSAGE = "Connection successful!";
const DELAY_PROFILES_SAVE_SUCCESS = "Delay profile sync config saved";

const LIDARR_RENAME_UNSUPPORTED_ERROR =
  "Rename is not supported for Lidarr instances";
const LIDARR_UPGRADES_UNSUPPORTED_ERROR =
  "Upgrades are not supported for Lidarr instances";
...
async function createArrInstance(
  ...
  await page.getByRole("button", { name: options.typeLabel, exact: true })
    .click();
  ...
  if (options.expectUnsupportedWorkflowStatus) {
    ...
    await expect(unsupportedWorkflowStatus).toContainText(
      LIDARR_SUPPORTED_WORKFLOW_MESSAGE,
    );
  } else {
...
function parseLibraryNumericParam(
  value: string | null,
  fallback: number,
): number {
...
function buildLidarrLibraryItems(query: string | null) {
  const allItems = Array.from({ length: 120 }, (_, index) => {
    ...
    return {
      ...
      albumType: "Album",
      releaseDate: `2025-${
        String((albumIndex % 12) + 1).padStart(2, "0")
      }-01T00:00:00.000Z`,
      status: "released",
      monitored: true,
      ...
      dateAdded: `2025-01-${
        String((albumIndex % 28) + 1).padStart(2, "0")
      }T00:00:00.000Z`,
      qualityProfileId: isPrimaryProfile ? 77 : 78,
      qualityProfileName: isPrimaryProfile
        ? LIDARR_LIBRARY_PROFILE
        : LIDARR_LIBRARY_ALT_PROFILE,
...
test.describe("2.40 Lidarr core flow", () => {
  ...
  test("add/test/configure/inspect flow with explicit Lidarr unsupported messaging", async ({ page }) => {
    ...
    const getLatestLibraryRequest = (): LibraryRequestInfo => {
      ...
      if (!latest) {
        throw new Error(
          "Expected at least one library request, but none were captured.",
        );
      }
    ...
    await page.route("**/api/v1/arr/library?*", async (route) => {
      ...
      if (
        url.searchParams.get("instanceId") !== String(trackedLidarrInstanceId)
      ) {
        await route.continue();
        return;
      }

      const pageParam = parseLibraryNumericParam(
        url.searchParams.get("page"),
        1,
      );
      const pageSizeParam = parseLibraryNumericParam(
      ...
      const totalPages = totalRecords > 0
        ? Math.ceil(totalRecords / pageSizeParam)
        : 0;
    ...
    expect(arrTestPayloads.some((payload) => payload.type === "lidarr")).toBe(
      true,
    );
    ...
    expect(arrTestPayloads.some((payload) => payload.type === "radarr")).toBe(
      true,
    );
    ...
    await expect(page.getByRole("columnheader", { name: "Tracks" }))
      .toBeVisible();
