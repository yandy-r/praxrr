...
} from '$lib/server/sync/customFormats/transformer.ts';
import { transformQualityProfile, type PcdQualityProfile } from '$lib/server/sync/qualityProfiles/transformer.ts';
...
Deno.test('lidarr custom format transform skips unsupported conditions with explicit reasons', () => {
  ...
  assertStringIncludes(transformed.skippedConditions[0].reason, 'not supported by Lidarr custom formats');
});
