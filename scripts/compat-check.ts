...
import { deleteCache, setCache, getCache } from '$pcd/database/registry.ts';
...
import { importBaseOps, __testOnly_resetCompile, __testOnly_setCompile } from '$pcd/ops/importBaseOps.ts';
...
function installQueryShims(databaseId: number): QueryStateResult {
  ...
  patch(
    ...
    ((searchDatabaseId: number, origin: 'base' | 'user', options?: { states?: PcdOpState[]; source?: PcdOpSource }) => {
      const byDatabase = operations.filter((operation) => operation.database_id === searchDatabaseId && operation.origin === origin);
      const bySource = options?.source ? byDatabase.filter((operation) => operation.source === options.source) : byDatabase;
      const byState =
  ...
  patch(
    ...
    ((id: number, input) => {
      ...
      if (input.desiredState !== undefined) op.desired_state = input.desiredState;
      if (input.contentHash !== undefined) op.content_hash = input.contentHash;
      if (input.lastSeenInRepoAt !== undefined) op.last_seen_in_repo_at = input.lastSeenInRepoAt;
      if (input.supersededByOpId !== undefined) op.superseded_by_op_id = input.supersededByOpId;
      if (input.pushedAt !== undefined) op.pushed_at = input.pushedAt;
      if (input.pushedCommit !== undefined) op.pushed_commit = input.pushedCommit;
      operations[index] = op;
      return true;
    }) as never
  );
