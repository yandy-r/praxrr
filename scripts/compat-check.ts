...
import type {
  CreatePcdOpHistoryInput,
  PcdOpHistory,
} from "$db/queries/pcdOpHistory.ts";
...
function installQueryShims(databaseId: number): QueryStateResult {
  ...
  const originalPcdOpsListByDatabaseAndOrigin =
    pcdOpsQueries.listByDatabaseAndOrigin;
  ...
  const originalPcdOpHistoryListLatestByDatabaseWithOps =
    pcdOpHistoryQueries.listLatestByDatabaseWithOps;
  const originalPcdOpHistoryListLatestConflictsByDatabase =
    pcdOpHistoryQueries.listLatestConflictsByDatabase;
  ...
  patch(
    databaseInstancesQueries as Record<string, unknown>,
    "getById",
    ((id: number): DatabaseInstance | undefined => {
      if (id === databaseId) {
        return fakeInstance;
      }
  ...
  patch(
    pcdOpsQueries as Record<string, unknown>,
    "create",
    ((input) => {
  ...
  patch(
    ...
    ((
      ...
      const byDatabase = operations.filter((operation) =>
        operation.database_id === searchDatabaseId &&
        operation.origin === origin
      );
      const bySource = options?.source
        ? byDatabase.filter((operation) => operation.source === options.source)
        : byDatabase;
      const byState = options?.states && options.states.length > 0
        ? bySource.filter((operation) =>
          options.states.includes(operation.state)
        )
        : bySource;
  ...
  patch(
    pcdOpsQueries as Record<string, unknown>,
    "getBaseByFilename",
    ((searchDatabaseId: number, filename: string) => {
      return operations.find(
        (op) =>
          op.database_id === searchDatabaseId && op.origin === "base" &&
  ...
  patch(
    pcdOpHistoryQueries as Record<string, unknown>,
    "create",
...
async function loadSchemaArtifacts(
  ...
  try {
    ...
    fail(
      "schema_ops",
      `Cannot access schema ops directory: ${schemaOpsPath}`,
      error,
    );
  }
...
async function validateSchemaLoads(
  ...
  try {
    for (const artifact of artifacts) {
      try {
        ...
        fail(
          "schema_ops",
          `Failed applying schema operation ${artifact.filename}: ${
            String(error)
          }`,
          error,
        );
...
async function createCompatPCDPath(): Promise<
  { path: string; cleanup: () => Promise<void> }
> {
  const schemaSource = path.join(Deno.cwd(), SCHEMA_OPS_DIR_PATH);
  const entitySource = path.join(
    Deno.cwd(),
    PCD_ENTITIES_ROOT_PATH,
    "entities",
  );
...
async function validateYamlEntities(pcdPath: string): Promise<void> {
  ...
  if (readerResult.issues.length > 0) {
    ...
    fail(
      "yaml_entities",
      `Failed to read YAML entities from ${pcdPath}/entities`,
      summary,
    );
  }

  const presentTypes = new Set(
    readerResult.candidates.map((candidate) => candidate.entityType),
  );
  const missing = ENTITY_TYPES.filter((entityType) =>
    !presentTypes.has(entityType)
  );
  if (missing.length > 0) {
    fail(
      "yaml_entities",
      `Missing YAML entities for supported types: ${missing.join(", ")}`,
    );
  }

  const presentPaths = new Set(
    readerResult.candidates.map((candidate) => candidate.relativePath),
  );
  const missingEntitySourcePaths = REQUIRED_MEDIA_MANAGEMENT_ENTITY_SOURCES
    .filter(
  ...
  if (missingEntitySourcePaths.length > 0) {
    fail(
      "yaml_entities",
      `Missing required media-management entity sources: ${
        missingEntitySourcePaths.join(", ")
      }`,
    );
  }
}
...
async function validateWritesCompile(pcdPath: string): Promise<void> {
  ...
  try {
    ...
    fail(
      "operation_writes",
      "YAML entity writes failed to compile into cache",
      error,
    );
  } finally {
    __testOnly_resetCompile();
...
async function main(): Promise<void> {
  const schemaArtifacts = await loadSchemaArtifacts(
    path.join(Deno.cwd(), SCHEMA_OPS_DIR_PATH),
  );
  ...
  try {
    ...
    try {
      ...
      console.error(
        "[compat-check] Temporary compatibility repo cleanup failed",
      );
    }
  }
}
