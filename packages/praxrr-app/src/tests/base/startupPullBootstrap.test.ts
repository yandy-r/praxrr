...
function runStartupEnqueueGate(
	upsertFn: (input: CreateJobQueueInput) => JobQueueRecord
): StartupEnqueueResult {
...
Deno.test('startup enqueue: startup remains non-blocking (enqueue returns synchronously)', () => {
	...
	try {
		...
		assertEquals(
			gateDuration < 50,
			true,
			`startup gate should return immediately (took ${gateDuration}ms)`
		);
	} finally {
		for (const restore of restores.reverse()) restore();
	}
