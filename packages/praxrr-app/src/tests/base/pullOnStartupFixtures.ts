...
import type {
  JobRunStatus,
  ArrPullStartupCounters,
  ArrPullStartupInstanceResult,
...
export function buildArrInstance(arrType: StartupPullArrType, overrides: ArrInstanceFixtureOptions = {}): ArrInstance {
...
export function buildSuccessAdapterEnvelope(overrides: CountersFixtureOptions = {}): StartupAdapterResultEnvelope {
...
export function buildRunSummary(overrides: RunSummaryFixtureOptions = {}): StartupPullRunSummary {
  const instances = overrides.instances ?? [];
  const counters = instances.length > 0 ? aggregateFixtureCounters(instances) : buildCounters(overrides);
...
export function buildEmptyRunSummary(overrides: RunSummaryFixtureOptions = {}): StartupPullRunSummary {
...
export function buildApplySelectionsResult(overrides: Partial<ApplySelectionsResult> = {}): ApplySelectionsResult {
  return {
 ...
 */
export function buildArrQualityProfile(id: number, name: string): Record<string, unknown> {
  return {
    id,
    name,
    upgradeAllowed: true,
 ...
 */
export function buildLidarrMetadataProfile(id: number, name: string): Record<string, unknown> {
  return {
    id,
    name,
    primaryAlbumTypes: [
