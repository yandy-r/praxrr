...
test.describe('4.4 Score Simulator UX basics', () => {
  ...
  test('decision summary appears after simulation and updates when overrides change total', async ({ page }) => {
    ...
    await expect(scoreBreakdown.getByText('This release meets your upgrade target.')).toBeVisible();
    ...
    await expect(scoreBreakdown.getByText(/Remaining to Minimum:\s*30/)).toBeVisible();
  });
