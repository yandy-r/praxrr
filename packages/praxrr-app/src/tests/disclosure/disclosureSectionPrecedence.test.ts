...
Deno.test('DisclosureSection precedence uses tier default when persisted=false', () => {
  ...
  const tierDriven = resolveTierDrivenMode('intermediate', tierToDefaultMode, beginner.hasExplicitInitialMode, false);
...
Deno.test('DisclosureSection precedence blocks tier updates when disclosure is persisted', () => {
  ...
  assertEquals(resolveTierDrivenMode('advanced', tierToDefaultMode, false, true), null);
});
...
Deno.test('DisclosureSection precedence blocks tier updates when initialMode is explicit', () => {
  ...
  assertEquals(resolveTierDrivenMode('advanced', tierToDefaultMode, hasExplicitInitialMode, false), null);
});
