...
Deno.test('pcdManager: hybrid base-op parse failures fall back to sql-only import', async () => {
  ...
  try {
    ...
    await Deno.writeTextFile(`${tempPath}/ops/001-base.sql`, 'CREATE TABLE IF NOT EXISTS fallback_guard (id INTEGER PRIMARY KEY);');
    ...
    await (pcdManager as unknown as { importBaseOpsWithOrchestration: (id: number, path: string) => Promise<void> })
