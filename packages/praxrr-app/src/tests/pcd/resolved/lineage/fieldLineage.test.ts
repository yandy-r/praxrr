...
const fileCell: EffectiveCell = {
  sourceLayer: 'tweaks',
  opId: null,
  opRef: { filename: '3.tweak.sql', order: 3 },
  ambiguous: false,
};
...
Deno.test('explainFieldLineage: explicit writer -> ${layer}-op even when value equals default (AC3)', () => {
  const r = explainFieldLineage({
    fieldPath: 'includeInRename',
    effectiveCell: baseCell,
    schemaDefault,
    valueMatchesDefault: true,
  });
...
Deno.test('explainFieldLineage: file-layer writer carries opRef, not opId', () => {
  const r = explainFieldLineage({
    fieldPath: 'x',
    effectiveCell: fileCell,
    schemaDefault: undefined,
    valueMatchesDefault: undefined,
  });
...
Deno.test('explainFieldLineage: ambiguous surviving writer makes no source claim (AC4)', () => {
  const r = explainFieldLineage({
    fieldPath: 'x',
    effectiveCell: { ...baseCell, ambiguous: true },
    schemaDefault,
    valueMatchesDefault: true,
  });
...
Deno.test(
  'explainFieldLineage: no writer + value differs from default -> ambiguous, never fabricated (AC4/AC7)',
  () => {
...
Deno.test('foldPendingConflict: pending forces every field ambiguous and entity status ambiguous', () => {
  ...
  assertEquals(
    r.lineage.every((f) => f.status === 'ambiguous' && f.sourceKind === 'ambiguous'),
    true
  );
});
