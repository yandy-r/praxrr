...
Deno.test('projection: delayProfile covers all diff paths', () => {
  ...
  assertCoversDiff(
    ...
    'delayProfile',
    undefined,
    base as ResolvedEntityPayload,
    resolved as ResolvedEntityPayload
  );
});
  ...
  const resolved = {
    ...
    conditions: [
      { name: 'Src', type: 'source', arrType: 'all', negate: true, required: false, sources: ['web'] },
      {
        ...
        negate: false,
        required: true,
        languages: [{ name: 'French', except: true }],
      },
      {
        name: 'Size',
        type: 'size',
...
Deno.test('projection: naming covers each Arr mapping (radarr/sonarr/lidarr)', () => {
  ...
  assertCoversDiff(
    ...
    'naming',
    'radarr',
    radarrBase as ResolvedEntityPayload,
    radarrResolved as ResolvedEntityPayload
  );

  const sonarrBase = {
    name: 'N',
    rename: true,
    standardEpisodeFormat: 'a',
    dailyEpisodeFormat: 'b',
    animeEpisodeFormat: 'c',
    seriesFolderFormat: 'd',
    seasonFolderFormat: 'e',
    replaceIllegalCharacters: true,
    colonReplacementFormat: 'smart',
  ...
  assertCoversDiff(
    ...
    'naming',
    'sonarr',
    sonarrBase as ResolvedEntityPayload,
    sonarrResolved as ResolvedEntityPayload
  );

  const lidarrBase = {
    name: 'N',
    rename: true,
    standardTrackFormat: 'a',
    artistName: 'b',
    multiDiscTrackFormat: 'c',
    artistFolderFormat: 'd',
    replaceIllegalCharacters: true,
    colonReplacementFormat: 'smart',
    customColonReplacementFormat: null,
  };
  ...
  assertCoversDiff(
    ...
    'naming',
    'lidarr',
    lidarrBase as ResolvedEntityPayload,
    lidarrResolved as ResolvedEntityPayload
  );
});
...
Deno.test('projection: qualityDefinitions covers entries for each Arr mapping (AC6 nested lists)', () => {
  for (const arrType of ['radarr', 'sonarr', 'lidarr'] as const) {
    const base = {
      name: 'QD',
      entries: [{ quality_name: 'WEBDL-1080p', min_size: 0, max_size: 100, preferred_size: 50 }],
    };
    const resolved = {
      name: 'QD',
      entries: [{ quality_name: 'WEBDL-1080p', min_size: 5, max_size: 100, preferred_size: 80 }],
    };
    assertCoversDiff(
      `${arrType} qualityDefinitions`,
      'qualityDefinitions',
      arrType,
      base as ResolvedEntityPayload,
      resolved as ResolvedEntityPayload
