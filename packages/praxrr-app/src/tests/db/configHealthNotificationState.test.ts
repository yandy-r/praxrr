...
migratedTest('registered migration creates the Config Health notification state table', () => {
  ...
  assertEquals(
    ...
    ['arr_instance_id', 'notified_signature', 'notified_at', 'created_at', 'updated_at']
  );
  assertEquals(columns.find((column) => column.name === 'arr_instance_id')?.pk, 1);
  assertEquals(columns.find((column) => column.name === 'notified_signature')?.notnull, 1);
  assertEquals(columns.find((column) => column.name === 'notified_at')?.notnull, 1);
});
...
migratedTest('first claim inserts state and get returns the diagnostic detail', () => {
  ...
  assertEquals(configHealthNotificationStateQueries.claim(instanceId, 'signature-a', FIRST_CLAIM_AT), true);
...
migratedTest('claim rejects an empty signature and invalid ISO UTC timestamp before writing', () => {
  ...
  assertThrows(
    () => configHealthNotificationStateQueries.claim(instanceId, '', FIRST_CLAIM_AT),
    TypeError,
    'signature must not be empty'
  );
  assertThrows(
    () => configHealthNotificationStateQueries.claim(instanceId, 'signature-a', 'not-a-timestamp'),
    TypeError,
    'valid ISO-8601 UTC timestamp'
  );
  assertThrows(
    () => configHealthNotificationStateQueries.claim(instanceId, 'signature-a', '2026-07-10T10:00:00+01:00'),
    TypeError,
    'valid ISO-8601 UTC timestamp'
  );
  assertEquals(configHealthNotificationStateQueries.get(instanceId), undefined);
...
migratedTest('identical claim is an atomic no-op and preserves every stored timestamp', () => {
  const instanceId = seedInstance();
  assertEquals(configHealthNotificationStateQueries.claim(instanceId, 'signature-a', FIRST_CLAIM_AT), true);
  const before = configHealthNotificationStateQueries.get(instanceId);
  assertExists(before);

  assertEquals(configHealthNotificationStateQueries.claim(instanceId, 'signature-a', CHANGED_CLAIM_AT), false);
  assertEquals(configHealthNotificationStateQueries.get(instanceId), before);
});
...
migratedTest('changed claim updates signature and timestamps while preserving created_at', () => {
  ...
  assertEquals(configHealthNotificationStateQueries.claim(instanceId, 'signature-b', CHANGED_CLAIM_AT), true);
...
migratedTest('overlapping identical claims have exactly one winner', async () => {
  ...
  const results = await Promise.all([
    Promise.resolve().then(() => configHealthNotificationStateQueries.claim(instanceId, 'signature-a', FIRST_CLAIM_AT)),
    Promise.resolve().then(() => configHealthNotificationStateQueries.claim(instanceId, 'signature-a', FIRST_CLAIM_AT)),
  ]);
...
migratedTest('clear removes current state and reports whether a row existed', () => {
  const instanceId = seedInstance();
  configHealthNotificationStateQueries.claim(instanceId, 'signature-a', FIRST_CLAIM_AT);

  assertEquals(configHealthNotificationStateQueries.clear(instanceId), true);
  assertEquals(configHealthNotificationStateQueries.get(instanceId), undefined);
  assertEquals(configHealthNotificationStateQueries.clear(instanceId), false);
});

migratedTest('claim enforces the arr_instances foreign key', () => {
  assertThrows(() => configHealthNotificationStateQueries.claim(999_999, 'signature-a', FIRST_CLAIM_AT));
