...
function patchConfig(overrides: Partial<MutableConfig>, restores: Restore[]): void {
...
Deno.test({
  ...
  fn: async () => {
    ...
    patchTarget(
      ...
      'getAll',
      () => [
        { id: 1, uuid: 'test-uuid', name: 'test-db', enabled: 1 } as ReturnType<
          typeof databaseInstancesQueries.getAll
        >[0],
      ],
      restores
    );
