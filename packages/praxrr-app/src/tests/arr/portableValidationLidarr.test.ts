...
Deno.test('validatePortableData: rejects legacy snake_case lidarr_media_settings propersRepacks values', () => {
  ...
  assertEquals(result, 'data.propersRepacks must be one of: doNotPrefer, preferAndUpgrade, doNotUpgradeAutomatically');
});
...
Deno.test('validatePortableData: rejects lidarr_media_settings payload missing required fields', () => {
  ...
  assertEquals(result, 'Unsupported payload for lidarr_media_settings: missing required fields: enableMediaInfo');
});
...
Deno.test('validatePortableData: rejects lidarr_quality_definitions payload with unsupported fields', () => {
  ...
  assertEquals(result, 'Unsupported payload for lidarr_quality_definitions: unsupported fields: standardEpisodeFormat');
});
