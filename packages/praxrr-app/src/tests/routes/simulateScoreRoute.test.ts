...
function createScoringFailureCache(errorMessage: string): PCDCache {
  return {
    kb: {
      selectFrom() {
        return {
...
Deno.test("simulate score: generic scoring errors containing not found still surface as 500", async () => {
  ...
  try {
    ...
    assertEquals(getErrorStatus(thrown), 500);
  } finally {
    trashGuideManager.listSources = originalListSources;
    deleteCache(databaseId);
    restore.restore();
...
Deno.test("simulate score: sonarr anime PCD profiles infer WEB source from matched fansub groups", async () => {
  ...
  const fixture = createPcdCacheFixture(`
    INSERT INTO quality_profiles (id, name, minimum_custom_format_score, upgrade_until_score, upgrade_score_increment)
    VALUES (1, 'Anime Heuristic Profile', 0, 0, 1);

    INSERT INTO custom_formats (name)
    VALUES ('Anime WEB');

    INSERT INTO regular_expressions (name, pattern)
    VALUES ('subsplease-regex', '(?<=^|[\\s.-])SubsPlease\\b');

    INSERT INTO custom_format_conditions (custom_format_name, name, type, arr_type, negate, required)
    VALUES ('Anime WEB', 'anime-source', 'source', 'all', 0, 1);
    INSERT INTO custom_format_conditions (custom_format_name, name, type, arr_type, negate, required)
    VALUES ('Anime WEB', 'anime-group', 'release_title', 'all', 0, 1);

    INSERT INTO condition_sources (custom_format_name, condition_name, source)
    VALUES ('Anime WEB', 'anime-source', 'web_dl');
    INSERT INTO condition_patterns (custom_format_name, condition_name, regular_expression_name)
    VALUES ('Anime WEB', 'anime-group', 'subsplease-regex');

    INSERT INTO quality_profile_custom_formats (quality_profile_name, custom_format_name, arr_type, score)
    VALUES ('Anime Heuristic Profile', 'Anime WEB', 'sonarr', 200);
  `);
