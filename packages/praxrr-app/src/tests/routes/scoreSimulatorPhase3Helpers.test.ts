...
Deno.test('createScoreOverrideEntry rounds finite values', () => {
  assertEquals(createScoreOverrideEntry('CF-One', 4.6), ['CF-One', 5]);
});

Deno.test('createScoreOverrideEntry rejects non-finite and non-number values', () => {
  assertEquals(createScoreOverrideEntry('CF-One', Number.NaN), undefined);
  assertEquals(createScoreOverrideEntry('CF-One', Number.POSITIVE_INFINITY), undefined);
  assertEquals(createScoreOverrideEntry('CF-One', '5'), undefined);
});
...
Deno.test('resolveThresholdWithOverrides transitions below to accepted at minimum threshold', () => {
  const profile = makeProfileScore('pcd:alpha', 7, 10, 20, [{ cfName: 'CF-One', score: 7 }]);
...
Deno.test('buildComparisonResult recalculates profile A totals and tracks original scores with overrides', () => {
  ...
  const overriddenContribution = comparison!.contributions.find((contribution) => contribution.cfName === 'CF-One');
  const unchangedContribution = comparison!.contributions.find((contribution) => contribution.cfName === 'CF-Two');
