      ...
      {#each displayData as row, index}
        ...
        >
          ...
          <div
            role="button"
            tabindex="0"
            aria-expanded={!shouldDisableExpand(row) ? expandedRows.has(rowId) : undefined}
          ...
          {#if !shouldDisableExpand(row) && expandedRows.has(rowId)}
            <div class="border-t border-neutral-200 bg-neutral-50 dark:border-neutral-700/60 dark:bg-neutral-900/30">
              <slot name="expanded" {row}>
                <div class="p-4 text-sm text-neutral-500 dark:text-neutral-400">No additional details</div>
              </slot>
            </div>
          {/if}
        </div>
...
{:else}
  ...
  >
    <table class="w-full">
      <thead class="border-b border-neutral-300 bg-neutral-50 dark:border-neutral-700/60 dark:bg-neutral-800/50">
        <tr>
          <!-- Expand column (left) -->
        ...
        {:else}
          {#each displayData as row, index}
            ...
            >
              ...
              {/each}
              {#if $$slots.actions}
                <td class="{compact ? 'px-4 py-2' : 'px-6 py-4'} text-right text-sm" on:click|stopPropagation>
                  <slot name="actions" {row} />
                </td>
              {/if}
                ...
                >
                  <div class={flushExpanded ? '' : 'ml-6'}>
                    <slot name="expanded" {row}>
                      ...
                      <div class="text-sm text-neutral-500 dark:text-neutral-400">No additional details</div>
                    </slot>
                  </div>
                </td>
              </tr>
