<script lang="ts">
  ...
  function normalizeSelection(
    ...
    return options.filter((option) => filteredKeys.includes(option.key)).map((option) => option.key);
  }
  ...
  {#if useDropdown}
    ...
    >
      ...
      >
        ...
        {/if}
        <ChevronDown size={16} class="text-neutral-400 transition-transform {dropdownOpen ? 'rotate-180' : ''}" />
      </button>
  ...
  {:else}
    <div class="flex" role="group" aria-label={ariaLabel}>
      {#each sourceOptions as option, index (option.key)}
        <button
          type="button"
          class="flex h-10 min-w-0 items-center gap-2 border border-neutral-300 px-3 text-sm font-medium transition-colors disabled:cursor-not-allowed disabled:opacity-50 dark:border-neutral-700/60 {index >
          0
            ? '-ml-px'
        ...
        >
          ...
          />
          <span class="max-w-40 truncate">{option.source.name}</span>
        </button>
      {/each}
    </div>
  {/if}
