<script lang="ts">
  ...
  import { ARR_CONDITION_TARGET_OPTIONS, supportsFeature, type ArrFeature } from '$shared/arr/capabilities.ts';
      ...
      {#each group.items as entry, index (entry.item.id)}
        ...
        {:else}
          <div class="mb-4">
            ...
            {/if}
            <div
              class="mb-1 flex items-center gap-2 rounded-lg border border-dashed border-neutral-300 px-3 py-2 text-sm text-neutral-600 dark:border-neutral-700 dark:text-neutral-400"
            >
              {#if !useEmoji}
