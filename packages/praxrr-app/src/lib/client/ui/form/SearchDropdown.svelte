<script lang="ts">
  ...
  function moveHighlight(direction: 1 | -1) {
    ...
    highlightedIndex = (highlightedIndex + direction + filteredOptions.length) % filteredOptions.length;
  }
    ...
    switch (event.key) {
      ...
      case 'Enter':
        if (open && filteredOptions.length > 0) {
          event.preventDefault();
          const index = highlightedIndex >= 0 && highlightedIndex < filteredOptions.length ? highlightedIndex : 0;
