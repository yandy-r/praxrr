<script lang="ts">
  ...
  export let color: 'accent' | 'blue' | 'green' | 'red' | 'neutral' | `#${string}` | `var(--${string})` = 'accent'; // accent, semantic colors, hex (#FFC230), or CSS var (var(--arr-radarr-color))
  ...
  $: resolvedOutlineClass = outlineClasses[resolvedColorKey as keyof typeof outlineClasses] ?? outlineClasses.accent;
  $: stateClass = checked
    ? variant === 'filled'
      ? isCustomColor
        ? customFilledClass
