 ...
 * any guard mismatch BEFORE persist), then persists all ops + recompiles once. A guard conflict →
 * `{ success:false }` and nothing lands — so scoring can never be committed while the ladder fails.
 */
