...
function parseCustomFormat(payload: unknown, arrType: TrashGuideSupportedArrType, filePath: string): TrashGuideCustomFormatEntity {
	...
	const rawScores = asRecord(readRequiredValue(record, 'trash_scores', filePath), `${filePath}: trash_scores`);
	...
	const rawSpecifications = asArray(readRequiredValue(record, 'specifications', filePath), `${filePath}: specifications`);
...
function parseCustomFormatSpecification(
	...
	const fields = asRecord(readRequiredValue(record, 'fields', `${filePath}: specifications[${index}]`), `${filePath}: specifications[${index}].fields`);
...
function parseQualityProfileFormatItems(record: Record<string, unknown>, filePath: string): TrashGuideQualityProfileFormatItem[] {
	...
	if (isRecord(value)) {
		return Object.entries(value)
			.map(([name, entryValue]) => {
				...
				throw new Error(
					`${filePath}: formatItems["${name}"] must be a numeric score or a 32-char trash_id reference`
				);
			})
...
function parseQualitySize(payload: unknown, arrType: TrashGuideSupportedArrType, filePath: string): TrashGuideQualitySizeEntity {
	const record = asRecord(payload, `${filePath}: root`);
...
function validateQualitySizeCompatibility(profileType: string, arrType: TrashGuideSupportedArrType, filePath: string): void {
...
function sortByIdentity(a: { readonly trash_id: string; readonly file_path: string }, b: { readonly trash_id: string; readonly file_path: string }): number {
