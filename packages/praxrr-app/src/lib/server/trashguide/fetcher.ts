...
async function readMetadata(metadataPath: string, localPath: string): Promise<TrashGuideMetadataDocument> {
	...
	try {
		...
		if (error instanceof Deno.errors.NotFound) {
			throw new TrashGuideFetcherError(
				'metadata_missing',
				`TRaSH metadata file not found: ${metadataPath}`,
				false,
				{
					operation: 'metadata',
...
function classifyGitError(
	...
	return new TrashGuideFetcherError('git_operation_failed', `Git ${details.operation} failed: ${message}`, true, baseDetails, {
		cause: error instanceof Error ? error : undefined,
	});
}
