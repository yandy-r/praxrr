...
export interface TransparencyFollowUpEntry extends TransparencyAuditEvidence {
  ...
  readonly rationale: string;
  readonly followUpUrl: `https://github.com/yandy-r/praxrr/issues/${number}`;
}

/**
 * The disposition union makes invalid ownership states unrepresentable:
 * follow-ups require an issue URL, while not-applicable entries require a
 * rationale and cannot carry a follow-up URL.
 */
export type TransparencyAuditEntry = TransparencyPassEntry | TransparencyNotApplicableEntry | TransparencyFollowUpEntry;

function pass(evidence: TransparencyAuditEvidence): TransparencyPassEntry {
  return {
    ...evidence,
    disposition: 'pass',
    rationale: null,
...
function syncEntry(section: string): TransparencyPassEntry {
  return pass({
    inputs: [
