...
import {
  ...
  NAMING_CHARACTER_REPLACEMENT_DEFAULTS_OP_SQL,
  NAMING_CHARACTER_REPLACEMENT_DEFAULTS_OP_VERSION,
} from '$db/migrations/20260224_normalize_naming_character_replacement_defaults.ts';
...
export async function seedBuiltInBaseOps(databaseId: number): Promise<SeedBuiltInBaseOpsResult> {
  const builtInOps = [
    ...
    {
      filename: NAMING_CHARACTER_REPLACEMENT_DEFAULTS_OP_FILENAME,
      opNumber: NAMING_CHARACTER_REPLACEMENT_DEFAULTS_OP_VERSION,
      sequence: NAMING_CHARACTER_REPLACEMENT_DEFAULTS_OP_VERSION,
      sql: NAMING_CHARACTER_REPLACEMENT_DEFAULTS_OP_SQL,
      metadata: NAMING_CHARACTER_REPLACEMENT_DEFAULTS_OP_METADATA,
    },
  ] as const;
