...
import { readMigrationEntitySources, type MigrationEntityCandidate } from '$pcd/migration/reader.ts';
...
function formatStableJson(value: unknown): string {
  ...
  if (value !== null && typeof value === 'object') {
    const entries = Object.keys(value as Record<string, unknown>)
      ...
      .map((key) => {
        const normalized = formatStableJson((value as Record<string, unknown>)[key]);
...
function compareRowsBySortKeys(a: Record<string, unknown>, b: Record<string, unknown>, keys: readonly string[]): number {
...
function sortCandidatesByImportOrder(candidates: readonly MigrationEntityCandidate[]): MigrationEntityCandidate[] {
...
async function buildEntitySnapshot(pcdPath: string): Promise<BuildResult> {
  ...
  const sortedCandidates = sortCandidatesByImportOrder(candidates.candidates);
