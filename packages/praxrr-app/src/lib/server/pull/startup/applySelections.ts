...
function getMatchedBySection(
  ...
  return matches.filter((m): m is StartupPullMatchedResult => m.section === section && m.status === 'matched');
}
...
function applyQualityProfiles(instanceId: number, matched: readonly StartupPullMatchedResult[]): ApplySectionOutcome {
...
function applyDelayProfiles(instanceId: number, matched: readonly StartupPullMatchedResult[]): ApplySectionOutcome {
  if (matched.length === 0) return NO_MATCHES;
...
function applyMediaManagement(
  ...
  const hasAny = namingMatches.length > 0 || mediaSettingsMatches.length > 0 || qualityDefinitionsMatches.length > 0;
  ...
  const newData = {
    ...
    mediaSettingsDatabaseId: mediaSettingsMatch ? mediaSettingsMatch.databaseId : current.mediaSettingsDatabaseId,
