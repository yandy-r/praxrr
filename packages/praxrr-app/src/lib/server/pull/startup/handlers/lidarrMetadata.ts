...
import { createStartupMetadataFingerprint } from '$lib/server/pull/startup/fingerprints.ts';
import { shouldSkipStartupDefault } from '$lib/server/pull/startup/defaultFilters.ts';
import type {
...
function toProfileToggle(value: { id: number; name: string; allowed: boolean }): LidarrMetadataProfileToggle {
  return {
    id: value.id,
    name: value.name,
...
function buildMetadataProfileFingerprint(input: LidarrMetadataProfileFingerprintInput): string | null {
...
function buildStartupMetadataProfileFingerprintFromRemote(profile: ArrLidarrMetadataProfile): string | null {
...
function buildStartupMetadataProfileFingerprintFromLocal(profile: LocalLidarrMetadataProfile): string | null {
  return buildMetadataProfileFingerprint({
    primaryAlbumTypes: profile.primaryAlbumTypes.map((entry) =>
      toProfileToggle({
        id: entry.typeId,
...
function sortMetadataProfiles(candidates: readonly StartupPullEntityDescriptor[]): StartupPullEntityDescriptor[] {
