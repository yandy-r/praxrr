...
type MediaManagementSection = Extract<
	StartupPullSection,
	'naming' | 'mediaSettings' | 'qualityDefinitions'
>;
...
function buildNamingFingerprintFromArr(
	arrType: StartupPullArrType,
...
function buildQualityDefinitionsFingerprintFromArr(
		...
		rows
			...
			.map(async (row) => {
				...
				return {
					...
					arrType,
					databaseId,
					fingerprint: localConfig
						? buildStartupNamingFingerprintFromLocal('naming', arrType, localConfig)
						: null,
...
export function classifyMediaManagementMatch(
	...
	if (skipDefault.skip) {
		return makeStartupMatchNoMatchResult(
			request,
			'default_skip' satisfies StartupPullMatchReason,
			{
