...
<div class="relative flex" on:mouseenter={handleMouseEnter} on:mouseleave={handleMouseLeave} role="group">
  <button
  ...
  >
    <div class="relative">
      ...
      {#if enabledCount < options.length}
        <div
          class="bg-accent-600 absolute -top-1 -right-1 flex h-3.5 w-3.5 items-center justify-center rounded-full text-[9px] font-bold text-white"
        >
          {enabledCount}
        </div>
      {/if}
  ...
  {#if isHovered}
    <div class="z-50" transition:fly={{ y: -8, duration: 150 }}>
      ...
      >
        <div class="px-3 py-2 text-xs font-medium text-neutral-500 dark:text-neutral-400">Search in...</div>
