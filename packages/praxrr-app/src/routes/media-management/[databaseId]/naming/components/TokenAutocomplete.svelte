<script lang="ts">
  ...
  $: flatTokens = categories.flatMap((cat) => cat.tokens.map((t) => ({ ...t, category: cat.name }))) as FlatToken[];

  $: query = '';
  $: filteredTokens = open ? flatTokens.filter((t) => t.token.toLowerCase().includes(query.toLowerCase())) : [];
  ...
  function moveHighlight(direction: 1 | -1) {
    ...
    highlightedIndex = (highlightedIndex + direction + filteredTokens.length) % filteredTokens.length;
  }
  ...
  function handleKeyDown(event: KeyboardEvent) {
    ...
    switch (event.key) {
      ...
      case 'Enter':
        if (filteredTokens.length > 0) {
          event.preventDefault();
          const index = highlightedIndex >= 0 && highlightedIndex < filteredTokens.length ? highlightedIndex : 0;
...
<div class="relative" use:clickOutside={close} on:keydown={handleKeyDown}>
  <FormInput {label} {name} {value} {placeholder} mono wrap bind:inputElement on:input={handleInput} />
