<script lang="ts">
  ...
  let demoRangeSingle: Marker[] = [{ id: 'threshold', label: 'Threshold', color: 'green', value: 60 }];
  ...
  function getVisibleIds(items: Section[], query: string): Set<string> {
    ...
    return new Set(
      items.filter((s) => s.name.toLowerCase().includes(q) || s.category.toLowerCase().includes(q)).map((s) => s.id)
    );
  }
</script>
      ...
      <div class="space-y-3">
        ...
        <CardGrid columns={3}>
          <Card>
            ...
            <p class="text-sm text-neutral-600 dark:text-neutral-400">
              Full disc remux with lossless audio. This card has more text to demonstrate equal row heights across the
              grid.
            </p>
            <svelte:fragment slot="footer">
  ...
  {#if visibleIds.has('dropdown')}
    ...
    >
      ...
      <div class="space-y-3">
        ...
        <div class="flex flex-wrap items-center gap-4">
          <DropdownSelect label="Arr type" bind:value={demoSelectValue} options={demoSelectOptions} position="left" />
          <DropdownSelect bind:value={demoSelectValue} options={demoSelectOptions} position="left" compact />
          <DropdownSelect bind:value={demoSelectValue} options={demoSelectOptions} position="left" disabled />
        </div>
      </div>
    </ComponentCard>
  {/if}
