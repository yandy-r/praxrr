<script lang="ts">
  ...
  import { buildParityRows, type ParityRow } from '$shared/arr/parityRows.ts';
  ...
  /** Narrow a column key to an Arr app type before indexing a row's status columns. */
  function isAppKey(key: string): key is ArrAppType {
    return (ARR_APP_TYPES as readonly string[]).includes(key);
  }
  ...
  const columns: Column<ParityRow>[] = [
    ...
    ...ARR_APP_TYPES.map((type): Column<ParityRow> => ({
      key: type,
