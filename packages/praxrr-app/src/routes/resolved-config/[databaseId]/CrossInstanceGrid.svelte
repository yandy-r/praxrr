<script lang="ts">
  ...
  /**
   * `instance.arrType` can be `null` for an incompatible instance (unrecognized `arr_type`
   * on the instance row) -- render an "unknown" badge instead of passing `null` through to
   * `Badge`'s closed `variant` union, and never key a `Record` by the raw value.
   */
  function resolveArrTypeBadge(instance: ResolvedInstanceState): {
    variant: 'radarr' | 'sonarr' | 'lidarr' | 'neutral';
    label: string;
  } {
    const arrType = instance.arrType ?? null;
    return arrType ? { variant: arrType, label: arrType } : { variant: 'neutral', label: 'unknown' };
  }
  ...
  function handleInstanceToggle(id: number, checked: boolean) {
    if (checked) {
      if (selectedInstanceIds.length >= COMPARE_MAX_INSTANCES) {
        capMessage = `You can compare up to ${COMPARE_MAX_INSTANCES} instances at a time -- deselect one first.`;
        return;
      }
  ...
  async function runCompare() {
    ...
    // Generation guard: if a newer runCompare starts before this one's awaits resolve,
    // this call's `requestId` no longer matches `compareRequestId` and its result/context
    ...
    try {
      ...
      if (response.status === 429) {
        ...
        const nextResult = (await response.json()) as CrossInstanceComparisonResponse;
        if (requestId !== compareRequestId) return;
        result = nextResult;
      }
    } catch (err) {
  ...
  <div class="space-y-3">
    <div class="flex flex-col gap-2">
      <span class="text-sm font-medium text-neutral-700 dark:text-neutral-300">
        Instances to compare (up to {COMPARE_MAX_INSTANCES})
      </span>
      <div class="flex flex-wrap gap-3">
        {#each filteredInstances as instance (instance.id)}
          {@const isSelected = selectedInstanceIds.includes(instance.id)}
          {@const atCap = !isSelected && selectedInstanceIds.length >= COMPARE_MAX_INSTANCES}
          <label
  ...
  {:else if displayResult}
    <div class="flex flex-wrap items-center gap-3">
      {#each displayResult.instances as instance (instance.instanceId)}
        ...
        {@const arrTypeBadge = resolveArrTypeBadge(instance)}
        <div class="flex items-center gap-2 rounded-lg border border-neutral-200 px-3 py-2 dark:border-neutral-800">
          ...
          <Badge variant={arrTypeBadge.variant}>{arrTypeBadge.label}</Badge>
