<script lang="ts">
  ...
  type DeleteResult = {
    ...
    failedCustomFormats: SkippedItem[];
  ...
  $: totalStale = scanResult ? scanResult.staleCustomFormats.length + scanResult.staleQualityProfiles.length : 0;
  ...
  <div slot="body">
        ...
        <div class="space-y-2">
          {#if scanResult.staleCustomFormats.length > 0}
            <p class="text-sm text-neutral-700 dark:text-neutral-300">
              <span class="font-medium">{scanResult.staleCustomFormats.length}</span> custom format{scanResult
                .staleCustomFormats.length === 1
                ? ''
                : 's'}
            </p>
          {/if}
          {#if scanResult.staleQualityProfiles.length > 0}
            <p class="text-sm text-neutral-700 dark:text-neutral-300">
              <span class="font-medium">{scanResult.staleQualityProfiles.length}</span> quality profile{scanResult
                .staleQualityProfiles.length === 1
                ? ''
                : 's'}
            </p>
          {/if}
    ...
    {:else if phase === 'results' && deleteResult}
      ...
      <div class="space-y-3">
          ...
          <div class="space-y-2">
            {#if deleteResult.deletedCustomFormats.length > 0}
              <div class="flex items-center gap-2">
                ...
                <p class="text-sm text-neutral-700 dark:text-neutral-300">
                  Deleted <span class="font-medium">{deleteResult.deletedCustomFormats.length}</span> custom format{deleteResult
                    .deletedCustomFormats.length === 1
                    ? ''
                    : 's'}
                </p>
              </div>
            ...
            {#if deleteResult.deletedQualityProfiles.length > 0}
              <div class="flex items-center gap-2">
                <Check size={16} class="flex-shrink-0 text-emerald-500" />
                <p class="text-sm text-neutral-700 dark:text-neutral-300">
                  Deleted <span class="font-medium">{deleteResult.deletedQualityProfiles.length}</span> quality profile{deleteResult
                    .deletedQualityProfiles.length === 1
                    ? ''
                    : 's'}
                </p>
              </div>
        ...
        {#if deleteResult.skippedQualityProfiles.length > 0}
          <div class="space-y-1">
            <div class="flex items-center gap-2">
              ...
              <p class="text-sm text-neutral-700 dark:text-neutral-300">
                Skipped {deleteResult.skippedQualityProfiles.length} profile{deleteResult.skippedQualityProfiles
                  .length === 1
                  ? ''
                  : 's'} assigned to media
              </p>
        ...
        {#if deleteResult.failedCustomFormats.length > 0}
          <div class="space-y-1">
            <div class="flex items-center gap-2">
              <AlertTriangle size={16} class="flex-shrink-0 text-amber-500" />
              <p class="text-sm text-neutral-700 dark:text-neutral-300">
                Skipped {deleteResult.failedCustomFormats.length} custom format{deleteResult.failedCustomFormats
                  .length === 1
                  ? ''
                  : 's'}
              </p>
            </div>
            {#each deleteResult.failedCustomFormats as failed}
              <p class="pl-6 text-sm text-neutral-500 dark:text-neutral-400">
                {failed.item.strippedName}: {failed.reason}
              </p>
            {/each}
          </div>
        {/if}
