  ...
  use:enhance={() => {
    return async ({ result, update }) => {
      if (result.type === 'failure' && result.data) {
        alertStore.add('error', (result.data as { error?: string }).error || 'Failed to delete instance');
...
<InfoModal bind:open={showInfoModal} header="Arr Instances">
  <div class="space-y-4 text-sm text-neutral-600 dark:text-neutral-400">
    <div>
      ...
      <div class="mt-1">
        Arr instances are your Radarr and Sonarr applications. Praxrr connects to these instances to sync quality
        profiles, custom formats, and other configurations.
      </div>
    </div>
    ...
    <div>
      ...
      <div class="mt-1">
        To add an instance, you'll need the URL and API key from your Radarr or Sonarr application. You can find the API
        key in Settings → General → Security.
      </div>
    </div>
    ...
    <div>
      ...
      <div class="mt-1">
        Once connected, you can configure sync settings to push profiles and formats from your linked databases to each
        instance. Sync can be triggered manually, on a schedule, or automatically when changes are detected.
      </div>
    </div>
    ...
    <div>
      ...
      <div class="mt-1">
        Disabled instances are excluded from sync operations but remain configured. This is useful for temporarily
        pausing sync without removing the instance.
      </div>
    </div>
  </div>
</InfoModal>
