  ...
  <div class="mb-4">
    <ActionsBar>
      ...
      <ActionButton icon={Filter} hasDropdown square title="Filter by filter name">
        <Dropdown slot="dropdown" position="right" mobilePosition="middle">
          <DropdownItem label="All filters" selected={filterFilter === 'all'} on:click={() => (filterFilter = 'all')} />
          {#each uniqueFilters as filter}
            <DropdownItem label={filter} selected={filterFilter === filter} on:click={() => (filterFilter = filter)} />
          {/each}
        </Dropdown>
      </ActionButton>
        ...
        <div class="flex">
          ...
          <span class="text-sm text-neutral-900 dark:text-neutral-100">
            {#if row.config.dryRun}
              ...
              <span class="{row.results.successful > 0 ? 'text-green-600 dark:text-green-400' : ''} font-mono"
              ...
              <svelte:fragment slot="expanded" let:row={item}>
                <div class="space-y-2 py-2 pl-2">
                  <div class="flex gap-2">
                    <span class="w-16 shrink-0 text-xs font-medium text-neutral-500 dark:text-neutral-400"
                      >Current:</span
                    >
                    <span
                  ...
                  {#if item.upgrade}
                    <div class="flex gap-2">
                      <span class="w-16 shrink-0 text-xs font-medium text-emerald-600 dark:text-emerald-400"
                        >Upgrade:</span
                      >
                      <span
                        class="truncate font-mono text-xs text-neutral-700 dark:text-neutral-300"
                  ...
                  {:else}
                    <div class="text-xs text-neutral-500 italic dark:text-neutral-400">No upgrade available</div>
                  {/if}
                </div>
              </svelte:fragment>
            </ExpandableTable>
