<script lang="ts">
	...
	function flattenSectionEntities(sectionChanges: readonly EntityChange[] | EntityChange | null): readonly EntityChange[] {
	...
	function collectSectionGroups(snapshot: SyncPreviewResult): SectionGroup[] {
		...
		if (snapshot.qualityProfiles) {
			const entities = [
				...snapshot.qualityProfiles.customFormats,
				...snapshot.qualityProfiles.qualityProfiles
			];
	...
	async function loadPreview(previewId: string) {
		...
		try {
			...
			if (!response.ok || !payload || !('id' in payload)) {
				loadError = (payload && 'error' in payload && typeof payload.error === 'string')
					? payload.error
	...
	async function handleApply() {
		...
		try {
			...
			const payload = (await response.json().catch(() => null)) as
				| { success: boolean; error?: string; staleWarning?: string }
				| { error: string; staleWarning?: string };
...
<div class="rounded-lg border border-neutral-200 bg-white dark:border-neutral-800 dark:bg-neutral-900">
	...
	<div class="space-y-4 p-4 md:p-6">
		...
		{:else if previewState.status === 'error'}
			<div class="rounded-md border border-red-200 bg-red-50 px-3 py-2 text-sm text-red-700 dark:border-red-900/40 dark:bg-red-900/20 dark:text-red-300">
		...
		{:else}
			<div class="space-y-4">
				...
				{#if staleness && staleness.shouldWarn && !staleness.shouldBlock}
					<div class="flex items-start gap-2 rounded-md border border-amber-200 bg-amber-50 px-3 py-2 text-sm text-amber-800 dark:border-amber-900/40 dark:bg-amber-900/20 dark:text-amber-200">
						<AlertTriangle size={16} class="mt-0.5" />
						<div>
							<p class="font-medium">Preview is stale.</p>
							<p>
								This snapshot is older than 5 minutes. Re-generating now is recommended before applying.
							</p>
						</div>
					</div>
				{/if}
				...
				{#if staleness?.shouldBlock}
					<div class="flex items-start gap-2 rounded-md border border-red-200 bg-red-50 px-3 py-2 text-sm text-red-700 dark:border-red-900/40 dark:bg-red-900/20 dark:text-red-300">
						<AlertTriangle size={16} class="mt-0.5" />
						<div>
				...
				{#if summary.totalCreates === 0 && summary.totalUpdates === 0 && summary.totalDeletes === 0 && summary.totalUnchanged > 0}
					<div class="flex items-start gap-2 rounded-md border border-emerald-200 bg-emerald-50 px-3 py-2 text-sm text-emerald-700 dark:border-emerald-900/40 dark:bg-emerald-900/20 dark:text-emerald-200">
				...
				{:else}
					<div class="space-y-3">
						{#each sectionGroups as sectionGroup}
							<div class="rounded-md border border-neutral-200 bg-neutral-50 p-3 dark:border-neutral-700 dark:bg-neutral-800/40">
				...
				<div class="flex flex-col gap-2 pt-2 sm:flex-row sm:items-center sm:justify-between">
					<div class="text-xs text-neutral-500 dark:text-neutral-400">
						{applyError ? `Apply error: ${applyError}` : 'Apply will run exactly the selected sync operations from this snapshot.'}
					</div>
...
>
	<div slot="body" class="space-y-3">
		...
		{#if staleness?.shouldWarn}
			<div class="rounded-md border border-amber-200 bg-amber-50 p-2 text-sm text-amber-800 dark:border-amber-900/40 dark:bg-amber-900/20 dark:text-amber-200">
