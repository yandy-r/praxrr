...
<div class="rounded-lg border border-neutral-200 bg-white p-4 dark:border-neutral-800 dark:bg-neutral-900">
  ...
  <div class="grid grid-cols-[auto_1fr] items-center gap-x-4 gap-y-3 md:flex md:flex-wrap md:gap-x-6">
    ...
    <div class="md:flex md:items-center md:gap-2">
      ...
      <DropdownSelect value={schedule} options={scheduleOptions} on:change={(e) => onScheduleChange?.(e.detail)} />
    </div>
  ...
  {#if lastRunAt}
    <div
      class="mt-4 flex flex-wrap items-center gap-3 border-t border-neutral-200 pt-4 text-xs text-neutral-500 md:mt-3 md:border-0 md:pt-0 dark:border-neutral-700 dark:text-neutral-400"
    >
      {#if !enabled}
        <span
