...
<Modal bind:open header={modalHeader} {confirmText} size="lg" on:cancel={handleCancel} on:confirm={handleConfirm}>
  <div slot="body" class="space-y-4">
    ...
    <div class="space-y-1">
      <span class="block text-sm font-medium text-neutral-700 dark:text-neutral-300"> Languages </span>
    ...
    <div class="space-y-1">
      <span class="block text-sm font-medium text-neutral-700 dark:text-neutral-300"> Indexers </span>
    ...
    <div class="space-y-1">
      ...
      <p class="text-xs text-neutral-500 dark:text-neutral-400">Press Enter to add flags (e.g., freeleech, scene)</p>
      ...
      use:enhance={() => {
        ...
        return async ({ result, update }) => {
          if (result.type === 'failure' && result.data) {
            alertStore.add('error', (result.data as { error?: string }).error || `Failed to ${mode} release`);
