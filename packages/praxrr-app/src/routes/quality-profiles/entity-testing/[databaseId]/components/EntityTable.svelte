...
>
  <svelte:fragment slot="cell" let:row let:column>
    {#if column.key === 'poster_path'}
      ...
      {:else}
        <div class="flex h-12 w-8 items-center justify-center rounded bg-neutral-200 dark:bg-neutral-700">
          {#if row.type === 'movie'}
        ...
        use:enhance={() => {
          return async ({ result, update }) => {
            if (result.type === 'failure' && result.data) {
              alertStore.add('error', (result.data as { error?: string }).error || 'Failed to delete entity');
