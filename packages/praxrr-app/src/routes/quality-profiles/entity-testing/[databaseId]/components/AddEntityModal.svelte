<script lang="ts">
  ...
  $: searchStore = getPersistentSearchStore(`entityTestingAddEntitySearch:${$page.params.databaseId}`);
    ...
    {#if !tmdbConfigured}
      <div class="rounded-lg border border-neutral-200 bg-neutral-50 p-4 dark:border-neutral-700 dark:bg-neutral-800">
        <p class="text-sm text-neutral-600 dark:text-neutral-300">
          ...
          <a href="/settings/general" class="text-accent-600 dark:text-accent-400 font-medium hover:underline"
            >Settings</a
          >
        ...
        >
          <svelte:fragment slot="dropdown" let:dropdownPosition>
            <Dropdown position={dropdownPosition}>
              ...
              />
              <DropdownItem label="Rating" selected={sortField === 'rating'} on:click={() => setSortField('rating')} />
              <DropdownItem label="Title" selected={sortField === 'title'} on:click={() => setSortField('title')} />
              <DropdownItem label="Year" selected={sortField === 'year'} on:click={() => setSortField('year')} />
            </Dropdown>
          </svelte:fragment>
        </ActionButton>
      </ActionsBar>
      ...
      {#if isSearching || activeQuery || results.length > 0}
        <div class="max-h-96 overflow-y-auto rounded-lg border border-neutral-200 dark:border-neutral-700">
          ...
          {:else if results.length === 0}
            <div class="p-8 text-center text-neutral-500 dark:text-neutral-400">No results found</div>
          {:else}
              ...
              {#each sortedResults as item}
                ...
                >
                  ...
                  <div class="h-24 w-16 flex-shrink-0 overflow-hidden rounded bg-neutral-200 dark:bg-neutral-700">
                    {#if item.posterPath}
                      <img src={getPosterUrl(item.posterPath)} alt={item.title} class="h-full w-full object-cover" />
                    {:else}
                  ...
                  <div class="flex min-w-0 flex-1 flex-col">
                    <div class="flex items-start justify-between gap-2">
                      ...
                      <div class="flex items-center gap-2">
                        ...
                        {/if}
                        {#if alreadyAdded}
                          <span class="text-xs font-medium text-neutral-500 dark:text-neutral-400"> Added </span>
                        {:else}
          ...
          <div class="flex flex-wrap gap-2">
            {#each Array.from(selectedItems.values()) as item}
              <button
                ...
                class="bg-accent-100 text-accent-800 hover:bg-accent-200 dark:bg-accent-900 dark:text-accent-200 dark:hover:bg-accent-800 flex items-center gap-1.5 rounded-full py-1 pr-1.5 pl-2 text-xs font-medium"
              >
                {#if item.type === 'movie'}
                  <Film size={12} />
                {:else}
