...
<div class="space-y-3">
              ...
              use:enhance={() => {
                return async ({ result, update }) => {
                  if (result.type === 'failure' && result.data) {
                    alertStore.add('error', (result.data as { error?: string }).error || 'Failed to delete release');
  ...
  <button
    ...
    class="hover:border-accent-300 hover:bg-accent-50/50 hover:text-accent-600 dark:hover:border-accent-600 dark:hover:bg-accent-900/10 dark:hover:text-accent-400 w-full rounded-lg border-2 border-dashed border-neutral-200 py-3 text-sm text-neutral-400 transition-colors dark:border-neutral-700"
  >
