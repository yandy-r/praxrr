<script lang="ts">
  ...
  $: selectedProfile = selectedProfileId ? data.qualityProfiles.find((p) => p.id === selectedProfileId) : null;
...
<div class="space-y-6 px-4 pt-4 pb-8 md:px-8">
  ...
  <ActionsBar className="w-full justify-center md:w-full md:mx-auto">
    ...
    <ActionButton icon={Sliders} hasDropdown={true} dropdownPosition="middle" square={!selectedProfile}>
      {#if selectedProfile}
    ...
    <ActionButton icon={Clapperboard} hasDropdown={true} dropdownPosition="middle">
      <Dropdown slot="dropdown" position="middle">
        <DropdownItem icon={Film} label="Movies" selected={moviesSelected} on:click={toggleMovies} />
        <DropdownItem icon={Tv} label="TV Series" selected={seriesSelected} on:click={toggleSeries} />
      </Dropdown>
    </ActionButton>
          ...
          <div class="flex flex-col items-center gap-2">
            ...
            <p class="text-neutral-600 dark:text-neutral-400">
              TMDB API key not configured. <a
                ...
                class="text-accent-600 dark:text-accent-400 hover:underline">Configure in Settings</a
              >
            </p>
          </div>
        {:else}
...
<InfoModal bind:open={showInfoModal} header="How Entity Testing Works">
  <div class="space-y-4 text-sm text-neutral-600 dark:text-neutral-400">
    <div>
      ...
      <p class="mt-1">
        Add movies or TV series from TMDB to use as test cases. These represent the media you want to simulate release
        matching for.
      </p>
    </div>
    <div>
      ...
      <p class="mt-1">
        For each entity, add test releases with realistic release titles (e.g., "Movie.2024.1080p.BluRay.REMUX-GROUP").
        You can also specify size, languages, indexers, and flags.
      </p>
    </div>
    <div>
      <div class="font-medium text-neutral-900 dark:text-neutral-100">Quality Profile Scoring</div>
      <p class="mt-1">
        Select a quality profile from the dropdown to see how each release would score. The score is calculated by
        matching custom formats and summing their configured point values.
      </p>
    </div>
    <div>
      <div class="font-medium text-neutral-900 dark:text-neutral-100">Custom Format Matching</div>
      <p class="mt-1">
        Expand a release row to see parsed metadata and which custom formats matched. Each matched format shows its
        score contribution, helping you understand why a release scored the way it did.
      </p>
    </div>
  </div>
</InfoModal>
