<script lang="ts">
  ...
  import { X, Check, ArrowUp, Info, Save, Loader2, Layers, ChevronUp, ChevronDown, Pencil } from 'lucide-svelte';
  ...
  function collapseGroup(group: OrderedItem) {
    ...
    const groupIndex = mainBucket.findIndex((item) => item.type === 'group' && item.name === group.name);
  ...
  function createGroup() {
    ...
    const insertIndex = mainBucket.findIndex((item) => item.type === 'quality' && selectedGroupNames.has(item.name));
    ...
    const newBucket = mainBucket.filter((item) => !(item.type === 'quality' && selectedGroupNames.has(item.name)));
...
<StickyCard position="top">
  <svelte:fragment slot="left">
    ...
    <p class="text-neutral-600 dark:text-neutral-400">Configure ordering, grouping, and upgrade behavior.</p>
  </svelte:fragment>
    ...
    {#each mainBucket as item, index (item.type === 'quality' ? `quality-${item.name}-${index}` : `group-${item.name}-${index}`)}
      ...
      >
        ...
        {#if hoverTargetIndex === index && willAddToGroup}
          <div
            class="border-accent-500 bg-accent-50/30 dark:border-accent-400 dark:bg-accent-950/30 pointer-events-none absolute inset-0 rounded-lg border-2 border-dashed"
          ></div>
        {/if}
        <div class="relative flex items-center justify-between">
          <div class="flex-1">
            <div class="font-medium text-neutral-900 dark:text-neutral-100">
              {#if item.type === 'group'}
                <span
                  class="hover:text-accent-600 dark:hover:text-accent-400 cursor-pointer"
            ...
            {#if item.type === 'group' && item.members}
              <div class="mt-1 hidden text-xs text-neutral-600 md:block dark:text-neutral-400">
          ...
          <div class="flex items-center gap-3">
            ...
            {:else if hoverTargetIndex === index && willAddToGroup}
              <div class="text-accent-600 dark:text-accent-400 text-xs font-medium">Add to Group</div>
            {/if}
            {#if item.type === 'group'}
        ...
        {#if item.type === 'group' && item.members}
          <div
            class="mt-3 border-t border-neutral-200 pt-3 text-xs text-neutral-600 md:hidden dark:border-neutral-700 dark:text-neutral-400"
          >
...
<InfoModal bind:open={showInfoModal} header="Qualities">
  <div class="space-y-4 text-sm text-neutral-600 dark:text-neutral-400">
    <div>
      ...
      <div class="mt-1">
        Define the order, grouping, and configuration of qualities. In previous versions, only enabled qualities were
        tracked. The new system stores all qualities (enabled and disabled) to maintain proper ordering across your
        entire quality profile. Missing qualities are automatically appended as disabled.
      </div>
    </div>
    ...
    <div>
      ...
      <div class="mt-1">
        Drag and drop qualities to change their priority. Higher positions indicate higher preference. The order
        determines which quality will be selected when multiple options are available.
      </div>
    </div>
    ...
    <div>
      ...
      <div class="mt-1">
        On desktop, hold Ctrl (or Cmd on Mac) while dragging a quality onto another quality to create a group. Groups
        allow multiple qualities to be treated as equal priority. You can also drag a quality onto an existing group to
        add it to that group. On mobile, use the Create Group button in the header.
      </div>
    </div>
    ...
    <div>
      ...
      <div class="mt-1">
        Click on a group name to edit it. Groups automatically get a default name when created, but you can customize
        this to better describe the qualities it contains.
      </div>
    </div>
    ...
    <div>
      ...
      <div class="mt-1">
        Click the X button on a group to break it apart into individual qualities. The qualities will maintain their
        enabled state and position.
      </div>
    </div>
    ...
    <div>
      ...
      <div class="mt-1">
        The blue checkbox indicates whether a quality is enabled. Click anywhere on the quality row or the checkbox
        itself to toggle. Disabled qualities are still tracked for ordering purposes but won't be used for downloads.
      </div>
    </div>
    ...
    <div>
      ...
      <div class="mt-1">
        The green arrow checkbox marks a quality as the "upgrade until" threshold. Only one quality can have this flag
        at a time. When a file reaches this quality level, the system will stop looking for upgrades. This quality must
        be enabled.
      </div>
    </div>
  </div>
