<script lang="ts">
  ...
  export let description = 'Select at least two qualities to combine into a group. Groups share the same priority.';
      ...
      {:else}
        {#each items as item}
          ...
          >
            ...
            <IconCheckbox checked={selectedNames.has(item.name)} icon={Check} color="blue" shape="circle" />
          </button>
        {/each}
      {/if}
    </div>
