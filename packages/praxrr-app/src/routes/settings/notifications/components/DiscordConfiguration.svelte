...
<div class="rounded-lg border border-neutral-200 bg-white p-6 dark:border-neutral-800 dark:bg-neutral-900">
  <h2 class="mb-4 text-lg font-semibold text-neutral-900 dark:text-neutral-50">Discord Configuration</h2>

  <div class="space-y-4">
    <!-- Webhook URL -->
    <div>
      <label for="webhook_url" class="block text-sm font-medium text-neutral-700 dark:text-neutral-300">
        Webhook URL
        {#if mode === 'create'}
    ...
    <div>
      ...
      <input
        ...
        class="mt-1 block w-full rounded-lg border border-neutral-300 bg-white px-3 py-2 text-sm text-neutral-900 placeholder-neutral-400 focus:border-neutral-400 focus:ring-1 focus:ring-neutral-400 focus:outline-none dark:border-neutral-700 dark:bg-neutral-800 dark:text-neutral-100 dark:placeholder-neutral-500 dark:focus:border-neutral-500 dark:focus:ring-neutral-500"
      />
      <p class="mt-1 text-xs text-neutral-500 dark:text-neutral-400">Custom username for the webhook bot</p>
    </div>

    <!-- Avatar URL (Optional) -->
    <div>
      <label for="avatar_url" class="block text-sm font-medium text-neutral-700 dark:text-neutral-300">
        Avatar URL (Optional)
      </label>
      <input
        type="url"
        ...
        class="mt-1 block w-full rounded-lg border border-neutral-300 bg-white px-3 py-2 text-sm text-neutral-900 placeholder-neutral-400 focus:border-neutral-400 focus:ring-1 focus:ring-neutral-400 focus:outline-none dark:border-neutral-700 dark:bg-neutral-800 dark:text-neutral-100 dark:placeholder-neutral-500 dark:focus:border-neutral-500 dark:focus:ring-neutral-500"
      />
      <p class="mt-1 text-xs text-neutral-500 dark:text-neutral-400">Custom avatar image for the webhook bot</p>
    </div>

    <!-- Enable Mentions -->
    <div class="flex items-start gap-3">
      <IconCheckbox icon={Check} checked={enableMentions} on:click={() => (enableMentions = !enableMentions)} />
      <input type="hidden" name="enable_mentions" value={enableMentions ? 'on' : ''} />
      <button type="button" class="flex-1 text-left" on:click={() => (enableMentions = !enableMentions)}>
        <span class="block text-sm font-medium text-neutral-700 dark:text-neutral-300"> Enable @here mentions </span>
        <p class="mt-1 text-xs text-neutral-500 dark:text-neutral-400">
