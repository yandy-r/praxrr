...
>
  <div class="space-y-6">
      ...
      <div class="space-y-2">
        ...
        <div class="flex items-center gap-3">
          ...
          <button type="button" class="flex-1 text-left" on:click={() => (settings.enabled = !settings.enabled)}>
            <span class="text-sm font-medium text-neutral-900 dark:text-neutral-50"> Enable Logging </span>
            <p class="text-xs text-neutral-500 dark:text-neutral-400">Master switch for all logging functionality</p>
          </button>
        </div>
        ...
        <div class="flex items-center gap-3">
          ...
          <button
            ...
            on:click={() => (settings.file_logging = !settings.file_logging)}
          >
            <span class="text-sm font-medium text-neutral-900 dark:text-neutral-50"> File Logging </span>
        ...
        <div class="flex items-center gap-3">
          ...
          <button
            ...
            on:click={() => (settings.console_logging = !settings.console_logging)}
          >
            <span class="text-sm font-medium text-neutral-900 dark:text-neutral-50"> Console Logging </span>
    ...
    <!-- Log Level -->
    <div>
      <label for="min_level" class="mb-2 block text-sm font-semibold text-neutral-900 dark:text-neutral-50">
        Minimum Log Level
      </label>
      <select
        id="min_level"
    ...
    <!-- Retention -->
    <div>
      <h3 class="mb-3 text-sm font-semibold text-neutral-900 dark:text-neutral-50">Retention Policy</h3>
      <div>
        <label for="retention_days" class="mb-1 block text-sm font-medium text-neutral-900 dark:text-neutral-50">
          Retention (days)
        </label>
        <NumberInput
          name="retention_days"
        ...
        />
        <p class="mt-1 text-xs text-neutral-500 dark:text-neutral-400">
          Keep daily log files for 1-365 days. Logs are automatically rotated daily (YYYY-MM-DD.log format).
        </p>
      </div>
    </div>
  </div>
