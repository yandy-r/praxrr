...
>
  <div class="space-y-6">
      ...
      <div class="space-y-2">
        <div class="flex items-center gap-3">
          ...
          <button type="button" class="flex-1 text-left" on:click={() => (settings.enabled = !settings.enabled)}>
            <span class="text-sm font-medium text-neutral-900 dark:text-neutral-50"> Enable Automatic Backups </span>
        ...
        <div class="flex items-center gap-3">
          ...
          <button
            ...
            on:click={() => (settings.compression_enabled = !settings.compression_enabled)}
          >
            <span class="text-sm font-medium text-neutral-900 dark:text-neutral-50"> Enable Compression </span>
            <p class="text-xs text-neutral-500 dark:text-neutral-400">Compress backups to save disk space</p>
          </button>
        </div>
      </div>
    </div>
    ...
    <div class="space-y-3">
      ...
      <div class="space-y-2">
        <label class="block">
          <span class="text-sm font-medium text-neutral-900 dark:text-neutral-50"> Schedule </span>
          <select
            name="schedule"
          ...
          <p class="mt-1 text-xs text-neutral-500 dark:text-neutral-400">How often to create automatic backups</p>
        </label>
      </div>
    </div>
    ...
    <!-- Retention Policy -->
    <div class="space-y-3">
      <h3 class="text-sm font-semibold text-neutral-900 dark:text-neutral-50">Retention Policy</h3>
      <div class="space-y-2">
        <label class="block">
          <span class="text-sm font-medium text-neutral-900 dark:text-neutral-50"> Retention Period (days) </span>
          <NumberInput name="retention_days" bind:value={settings.retention_days} min={1} max={365} />
          <p class="mt-1 text-xs text-neutral-500 dark:text-neutral-400">Delete backups older than this many days</p>
        </label>
      </div>
    </div>
    ...
    <div class="flex justify-end gap-3 border-t border-neutral-200 pt-6 dark:border-neutral-800">
      <button
        type="submit"
        class="bg-accent-600 hover:bg-accent-700 dark:bg-accent-500 dark:hover:bg-accent-600 flex items-center gap-2 rounded-lg px-4 py-2 text-sm font-medium text-white transition-colors"
      >
        <Save size={16} />
        Save Changes
      </button>
