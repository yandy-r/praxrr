<script lang="ts">
  ...
  export let form: { success?: boolean; error?: string } | null | undefined = undefined;
  ...
  onMount(() => {
    if (mode === 'edit' && source) {
      initEdit({
        ...
        autoPull: String(source.autoPull),
      });
    } else {
      initCreate({
        arrType: '',
  ...
  function handleSave() {
    ...
    saving = true;
    savingStatus =
      mode === 'create' ? 'Cloning TRaSH Guides repository and parsing configurations...' : 'Saving settings...';
  ...
  <div class="space-y-6">
    <StickyCard position="top">
      ...
      <svelte:fragment slot="right">
        <Button text="Adding..." icon={Save} iconColor="text-blue-600 dark:text-blue-400" disabled={true} />
      </svelte:fragment>
    </StickyCard>
    ...
    >
      ...
      <div class="text-center">
        <p class="font-medium text-neutral-900 dark:text-neutral-50">Setting up TRaSH Guides</p>
        <p class="mt-1 text-sm text-neutral-500 dark:text-neutral-400">
          {savingStatus}
        </p>
        <p class="mt-2 text-xs text-neutral-400 dark:text-neutral-500">This may take a minute on first setup</p>
      </div>
    </div>
  </div>
{:else}
  <div class="space-y-6" class:mt-6={mode === 'edit'}>
    ...
    >
      <!-- Arr Type -->
      <div class="space-y-2">
        <span class="block text-sm font-medium text-neutral-900 dark:text-neutral-100"> Arr Type </span>
        <p class="text-xs text-neutral-500 dark:text-neutral-400">Select the Arr application this source is for</p>
        <DropdownSelect
          value={arrType}
      ...
      <!-- Score Profile -->
      <div class="space-y-2">
        <span class="block text-sm font-medium text-neutral-900 dark:text-neutral-100"> Score Profile </span>
        <p class="text-xs text-neutral-500 dark:text-neutral-400">Score set to use for custom format scoring</p>
        <DropdownSelect
          value={scoreProfile}
      ...
      <div class="space-y-2">
        ...
        <p class="text-xs text-neutral-500 dark:text-neutral-400">How often to check for updates from TRaSH Guides</p>
        <DropdownSelect
          value={syncStrategy}
      ...
      <div class="space-y-2">
        ...
        <p class="text-xs text-neutral-500 dark:text-neutral-400">Automatically pull updates when available</p>
    ...
    use:enhance={() => {
      ...
      return async ({ result, update: formUpdate }) => {
        if (result.type === 'failure' && result.data) {
          alertStore.add('error', (result.data as { error?: string }).error || 'Failed to unlink source');
