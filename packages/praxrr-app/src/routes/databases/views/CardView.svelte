<script lang="ts">
	...
	function formatEntityCount(item: UnifiedDatabaseItem): string {
		...
		if (item.entityCounts.customFormats > 0)
			parts.push(`${item.entityCounts.customFormats} CF`);
		if (item.entityCounts.qualityProfiles > 0)
			parts.push(`${item.entityCounts.qualityProfiles} QP`);
		if (item.entityCounts.qualitySizes > 0)
			parts.push(`${item.entityCounts.qualitySizes} QS`);
