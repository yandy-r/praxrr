	...
	<div class="flex min-h-[60vh] items-center justify-center p-4 sm:p-8">
		<div class="max-w-lg text-center">
			...
			<div class="mt-6 flex flex-col items-center gap-3 sm:flex-row sm:justify-center">
				<Button
					text="Add TRaSH Guides"
					icon={Sparkles}
					variant="primary"
					href="/databases/new/trash-guide"
				/>
				<Button
					text="Link Custom Database"
					icon={Database}
					variant="secondary"
					href="/databases/new/custom"
				/>
			</div>
			<div class="mt-4">
				<a
	...
	use:enhance={() => {
		return async ({ result, update }) => {
			...
			if (result.type === 'failure' && result.data) {
				alertStore.add(
					'error',
					(result.data as { error?: string }).error || 'Failed to unlink TRaSH source'
				);
			} else if (result.type === 'redirect') {
...
>
	<input
		type="hidden"
		name="id"
		value={selectedItem?.type === 'trash' ? selectedItem?.id : ''}
	/>
</form>
...
<InfoModal bind:open={showInfoModal} header="Data Sources">
	<div class="space-y-4 text-sm text-neutral-600 dark:text-neutral-400">
		<div>
			...
			<div class="mt-1">
				Data sources provide quality profiles, custom formats, and other configurations. You
				can link multiple sources and sync them to your Arr instances.
			</div>
		</div>
		...
		<div>
			...
			<div class="mt-1">
				TRaSH Guides provide curated quality profiles and custom formats for Radarr and
				Sonarr. They are community-maintained and regularly updated with best practices.
			</div>
		</div>
		...
		<div>
			...
			<div class="mt-1">
				Praxrr Compliant Database (PCD) repositories contain quality profiles, custom
				formats, and other configurations. Link a database to import and sync configurations
		...
		<div>
			...
			<div class="mt-1">
				...
				<strong>Dev</strong> means you have a personal access token configured, allowing you
		...
		<div>
			...
			<div class="mt-1">
				Controls how often Praxrr checks for updates from the remote repository. Set to
		...
		<div>
			...
			<div class="mt-1">
				Unlinking a source removes all local data associated with it. Your Arr instances will
				keep any configurations that were already synced, but you won't be able to sync
				updates until you re-link.
			</div>
		</div>
	</div>
</InfoModal>
