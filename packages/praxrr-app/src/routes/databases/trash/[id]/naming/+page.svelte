<script lang="ts">
  ...
  const SOURCE_FILTER_STORAGE_PREFIX = 'trashNamingSourceFilter';
  ...
  let initialSort = null as { key: string; direction: 'asc' | 'desc' } | null;
  let sortState: { key: string; direction: 'asc' | 'desc' } | null = null;

  $: source = data.source;
  $: namingConfigs = data.namingConfigs ?? [];
  ...
  $: filtered = $debouncedQuery
    ? namingConfigs.filter((nc: SourcedNamingListItem) => nc.name.toLowerCase().includes($debouncedQuery.toLowerCase()))
    : namingConfigs;
  ...
  ].filter(Boolean) as Column<SourcedNamingListItem>[];
  ...
  $: if (!initializedFromUrl) {
    ...
    if (key === 'name' || key === 'sourceDatabaseName') {
      initialSort = {
        key,
  ...
  function handleSortChange(nextSort: { key: string; direction: 'asc' | 'desc' } | null): void {
    sortState = nextSort;
  }
...
<div class="mt-6 space-y-4">
  ...
  {/if}
  {#if namingConfigs.length === 0}
    <div
      class="rounded-lg border border-neutral-200 bg-white p-8 text-center dark:border-neutral-800 dark:bg-neutral-900"
    >
      <p class="text-neutral-600 dark:text-neutral-400">No naming configs cached. Try syncing sources.</p>
    </div>
