...
<div class="mt-6">
  {#if scoringRows.length === 0}
    ...
    >
      <p class="text-neutral-600 dark:text-neutral-400">No custom format scores defined for this profile.</p>
    </div>
  {:else}
    <Table {columns} data={scoringRows} compact emptyMessage="No scoring entries" responsive />
  {/if}
</div>
