<script lang="ts">
  ...
  const storageKey = 'dependencyGraphDatabase';
  ...
  onMount(() => {
    ...
    goto(`/dependency-graph/${targetId}`);
  });
</script>
...
{#if data.databases.length === 0}
  <EmptyState
    ...
    description="Link a Praxrr Compliant Database to explore its configuration dependency graph."
