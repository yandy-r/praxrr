<script lang="ts">
  ...
  const NODE_KIND_FILTERS: { value: string; label: string }[] = [
    ...
    ...(
      ['custom_format', 'quality_profile', 'regular_expression', 'quality', 'quality_definition'] as GraphNodeKind[]
    ).map((kind) => ({ value: kind, label: NODE_META[kind].label })),
  ];
...
<div class="space-y-8">
  <div>
    ...
    <p class="mt-1 text-sm text-neutral-500 dark:text-neutral-400">
      Resolved dependencies between config entities — which custom formats are scored by which quality profiles, which
      regular expressions each custom format uses, and which qualities a profile enables.
    </p>
  </div>
  ...
  {:else if graph}
    ...
    {/if}
    <AdjacencyTable nodes={graph.nodes} edges={graph.edges} databaseId={graph.databaseId} {focusKey} />
  {/if}
</div>
