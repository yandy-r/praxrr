...
>
	<svelte:fragment slot="cell" let:row let:column>
		{#if column.key === 'name'}
			...
			{#if href}
				<a class="font-medium text-accent-700 hover:underline dark:text-accent-300" {href}>{row.name}</a>
			{:else}
					...
					<ul class="flex flex-col gap-1.5">
						{#each neighbors.dependsOn as neighbor (refKey(neighbor.kind, neighbor.name))}
							...
							<li class="flex flex-wrap items-center gap-2 text-sm">
								...
								{#if href}
									<a class="text-accent-700 hover:underline dark:text-accent-300" {href}>{neighbor.name}</a>
								{:else}
