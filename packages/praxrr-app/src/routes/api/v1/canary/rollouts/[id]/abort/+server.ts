...
/**
 * POST /api/v1/canary/rollouts/{id}/abort
 *
 * Cancel the verification gate (issue #19): transition the rollout from
 * `awaiting_confirmation` to `aborted` so the remaining instances are never dispatched. The
 * body must carry the `stateToken` shown at the gate as a value-guard; a stale token (the
 * gate was refreshed or already actioned) is rejected 422 and the caller must refresh and
 * retry. The canary write already applied is untouched — recover via snapshot / rollback.
 */
export const POST: RequestHandler = async ({ params, request }) => {
  ...
  try {
    const rollout = abortRollout(idResult.value, stateToken);
    return json(rollout);
  } catch (error) {
    ...
    await logger.error('Failed to abort canary rollout', {
      source: 'CanaryAbortRoute',
      meta: {
    ...
    return json({ error: 'Failed to abort canary rollout' }, { status: 500 });
  }
};
