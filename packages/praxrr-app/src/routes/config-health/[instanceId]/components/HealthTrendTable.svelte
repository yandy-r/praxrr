  ...
  <div class="flex flex-wrap items-end justify-between gap-2">
    <div>
      ...
      <p class="mt-1 text-xs text-neutral-500 dark:text-neutral-400">
        {result.counts.points} persisted snapshot{result.counts.points === 1 ? '' : 's'} in canonical chronological order.
        Times are shown in {displayedTimeZone}.
      </p>
    </div>
    ...
    >
      <table class="w-full min-w-[64rem] border-collapse text-left text-sm">
        <caption class="sr-only">
          Every Config Health point in server-provided chronological order, including snapshot identity, engine
        ...
        <tbody class="divide-y divide-neutral-200 dark:divide-neutral-800">
          {#each result.points as point, pointIndex (point.snapshotId)}
            <tr
            ...
            >
              ...
              <td class="px-4 py-4 text-neutral-700 dark:text-neutral-300">
                ...
                {#if boundaryAt(pointIndex)}
                  ...
                  >
                    {pointIndex === 0 ? 'Series starts' : 'Engine changed'} here
                  </span>
                {/if}
              </td>
              <td class="px-4 py-4">
