...
<script lang="ts">
  ...
  const storageKey = 'impactSimulatorDatabase';
  ...
  onMount(() => {
    ...
    const isValidStored = Number.isFinite(stored) && data.databases.some((database) => database.id === stored);
    const targetId = isValidStored ? stored : data.databases[0].id;

    redirecting = true;
    localStorage.setItem(storageKey, String(targetId));
    void goto(resolve('/impact-simulator/[databaseId]', { databaseId: String(targetId) }));
  });
</script>

<svelte:head>
  <title>Impact Simulator - Praxrr</title>
</svelte:head>
...
{#if data.databases.length === 0}
  <EmptyState
    ...
    description="Link a Praxrr Compliant Database to use the Impact Simulator."
