<script lang="ts">
  ...
  $: overriddenThresholdState = profileScore ? resolveThresholdWithOverrides(profileScore, overrides) : null;
...
<div class="rounded-lg border border-neutral-200 bg-white p-4 dark:border-neutral-800 dark:bg-neutral-900">
  {#if !profileScore}
    <div class="py-6 text-sm text-neutral-500 dark:text-neutral-400">Select a profile to see score breakdown.</div>
  {:else}
    <div class="space-y-4">
      <div class="flex flex-wrap items-center justify-between gap-3">
        <div>
          <div class="text-xs tracking-wide text-neutral-500 uppercase dark:text-neutral-400">Total Score</div>
      ...
      <div class="flex flex-wrap gap-2">
        ...
        <Badge variant="neutral" size="md" mono={true}>Upgrade Until: {upgradeUntilScore.toLocaleString()}</Badge>
      </div>

      {#if activeThresholdState}
        <div
          class="rounded-md border border-neutral-200 bg-neutral-50 p-3 dark:border-neutral-800 dark:bg-neutral-950/60"
        >
        ...
        <div class="mb-2 flex items-center justify-between gap-2">
          ...
          {#if hasOverrides}
            <div class="flex items-center gap-2">
              <span
                class="rounded-full bg-amber-100 px-2 py-0.5 text-[10px] font-medium tracking-wide text-amber-800 uppercase dark:bg-amber-900/40 dark:text-amber-200"
              ...
              {#if onOverrideResetAll}
                <button
                  type="button"
                  class="focus-visible:ring-accent-400 inline-flex items-center gap-1 rounded-md border border-amber-300 bg-white px-2 py-1 text-xs font-medium text-amber-900 transition-colors hover:bg-amber-100 focus-visible:ring-2 focus-visible:outline-none dark:border-amber-800 dark:bg-amber-950 dark:text-amber-100 dark:hover:bg-amber-900/70"
        ...
        {:else}
          <ul class="space-y-2">
            {#each overriddenContributions as contribution (contribution.cfName)}
              ...
              {@const scoreDelta = hasOverride ? contribution.score - (contribution.originalScore ?? 0) : 0}
              <li
              ...
              >
                ...
                <div class="flex items-center gap-2">
                  ...
                  {:else}
                    <button
                      type="button"
                      class="focus-visible:ring-accent-400 rounded px-1 py-0.5 text-left transition-colors hover:bg-neutral-100 focus-visible:ring-2 focus-visible:outline-none dark:hover:bg-neutral-800"
                  ...
                  {#if hasOverride}
                    ...
                    <span
                      class="font-mono text-xs {scoreDelta > 0
                    ...
                    {#if onOverrideReset}
                      <button
                        type="button"
                        class="focus-visible:ring-accent-400 inline-flex items-center rounded border border-amber-300 bg-white p-1 text-amber-900 transition-colors hover:bg-amber-100 focus-visible:ring-2 focus-visible:outline-none dark:border-amber-800 dark:bg-amber-950 dark:text-amber-100 dark:hover:bg-amber-900/70"
