<script lang="ts">
  ...
  $: description =
    mode === 'create' ? `Create metadata profiles for ${databaseName}` : 'Update metadata profile settings';
  ...
  $: secondaryAllAllowed = secondaryTypes.length > 0 && secondaryAllowedCount === secondaryTypes.length;
  $: releaseAllAllowed = releaseStatuses.length > 0 && releaseAllowedCount === releaseStatuses.length;

  $: canSave = name.length > 0 && hasPrimaryAllowed && hasSecondaryAllowed && hasReleaseAllowed;
    ...
    use:enhance={() => {
      ...
      return async ({ result, update: formUpdate }) => {
        if (result.type === 'failure' && result.data) {
          ...
          alertStore.add('success', mode === 'create' ? 'Metadata profile created!' : 'Metadata profile updated!');
              ...
              <div class="flex items-center justify-between gap-3">
                ...
                <div class="flex flex-col items-end gap-1">
                  ...
                  <Toggle
                    ...
                    on:change={(event: CustomEvent) => setAllTypes('primaryTypes', event.detail)}
                  />
                </div>
              </div>
                ...
                {:else}
                  {#each primaryTypes as typeEntry, index (typeEntry.id)}
                    <Toggle
                      ...
                      on:change={(event: CustomEvent) => updateType('primaryTypes', index, event.detail)}
                    />
                  {/each}
                {/if}
              </div>
              ...
              <div class="flex items-center justify-between gap-3">
                ...
                <div class="flex flex-col items-end gap-1">
                  ...
                  <Toggle
                    ...
                    on:change={(event: CustomEvent) => setAllTypes('secondaryTypes', event.detail)}
                  />
                </div>
              </div>
              <div class="grid gap-2">
                ...
                {:else}
                  {#each secondaryTypes as typeEntry, index (typeEntry.id)}
                    <Toggle
                      label={typeEntry.name}
                      checked={typeEntry.allowed}
                      on:change={(event: CustomEvent) => updateType('secondaryTypes', index, event.detail)}
                    />
                  {/each}
                {/if}
              </div>
              ...
              <div class="flex items-center justify-between gap-3">
                ...
                <div class="flex flex-col items-end gap-1">
                  ...
                  <Toggle
                    ...
                    on:change={(event: CustomEvent) => setAllTypes('releaseStatuses', event.detail)}
                  />
                </div>
              </div>
              <div class="grid gap-2">
                ...
                {:else}
                  {#each releaseStatuses as typeEntry, index (typeEntry.id)}
                    <Toggle
                      label={typeEntry.name}
                      checked={typeEntry.allowed}
                      on:change={(event: CustomEvent) => updateType('releaseStatuses', index, event.detail)}
                    />
                  {/each}
                {/if}
              </div>
  ...
  {#if mode === 'edit'}
    <form
      ...
      use:enhance={() => {
        ...
        return async ({ result, update: formUpdate }) => {
          if (result.type === 'failure' && result.data) {
            alertStore.add('error', (result.data as { error?: string }).error || 'Failed to delete profile');
          } else if (result.type === 'redirect') {
