...
2. **Phase 2 – Instance settings & actions**: add the optional `External URL` input, hidden form plumbing, dirty tracking, and logging plus the server-side parsing/validation in `packages/praxrr-app/src/routes/arr/new/+page.server.ts` and `packages/praxrr-app/src/routes/arr/[id]/settings/+page.server.ts`. This phase depends on Phase 1 for query signatures but splits cleanly into UI and action/parsing workstreams that can run concurrently as soon as the data contract is ready.
...
| File                                                                         | Task(s)                                                                                                                                                                                                                             |
| ---------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `packages/praxrr-app/src/lib/server/db/migrations.ts`                                            | Register the new `external_url` migration so it runs alongside existing schema changes.                                                                                                                                             |
| `packages/praxrr-app/src/lib/server/db/migrations/0XX_add_arr_instances_external_url.ts` _(new)_ | `ALTER TABLE arr_instances ADD COLUMN external_url TEXT NULL`, preserve `updated_at` semantics if needed, and ensure any default behavior keeps prior rows unchanged.                                                               |
| `packages/praxrr-app/src/lib/server/db/schema.sql`                                               | Document `external_url` next to `url` in the `arr_instances` definition so the canonical schema reference matches production.                                                                                                       |
| `packages/praxrr-app/src/lib/server/db/queries/arrInstances.ts`                                  | Expose `external_url` on `ArrInstance`, `CreateArrInstanceInput`, `UpdateArrInstanceInput`; wire it through `INSERT`/`UPDATE`, normalize blank strings to `null`, and ensure helper callers (create/update actions) pass the value. |
| `packages/praxrr-app/src/routes/arr/components/InstanceForm.svelte`                              | Add the `External URL (optional)` field, helper copy, dirty-state seeding for edits, and a hidden `<input name="external_url">` so the enhanced form sends the value.                                                               |
| `packages/praxrr-app/src/routes/arr/new/+page.server.ts`                                         | Trim/validate the optional field, pass it to `arrInstancesQueries.create`, and normalize empty strings to `null` in logs/values.                                                                                                    |
| `packages/praxrr-app/src/routes/arr/[id]/settings/+page.server.ts`                               | Parse `external_url`, include it in the update payload, and keep logging of `url` vs `external_url` for debugging.                                                                                                                  |
| `packages/praxrr-app/src/routes/arr/[id]/+layout.server.ts`                                      | No code change, but confirm it continues returning the `instance` row so all descendant routes see `external_url` once the query layer includes it.                                                                                 |
| `packages/praxrr-app/src/routes/arr/[id]/library/+page.svelte`                                   | Compute `baseUrl`/`handleOpen` from the shared resolver, metadata for `LibraryActionBar`, and re-use the resolved URL for the `/movie`, `/series`, and `/artist` anchors.                                                           |
| `packages/praxrr-app/src/routes/arr/[id]/library/components/LibraryActionBar.svelte`             | Invoke the resolved URL in `onOpen` so the toolbar button matches row-level links.                                                                                                                                                  |
| `packages/praxrr-app/src/routes/arr/views/CardView.svelte`                                       | Use the resolver for the `ExternalLink` button and consider showing a helper badge if desired (but avoid exposing both URLs).                                                                                                       |
| `packages/praxrr-app/src/routes/arr/views/TableView.svelte`                                      | Update the `TableActionButton` to call `window.open` with the resolved URL instead of `instance.url`, and keep the `Badge` display limited to the canonical `url` if that matches existing UX.                                      |
| `packages/praxrr-app/src/tests/...` _(targeted)_                                                 | Add regression coverage for create/update actions (with/without `external_url`) and for UI fallback behavior if feasible (unit/visual tests for library and list surfaces).                                                         |
