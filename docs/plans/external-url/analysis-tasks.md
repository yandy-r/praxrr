...
| `src/lib/server/db/migrations/0XX_add_arr_instances_external_url.ts` *(new)* | `ALTER TABLE arr_instances ADD COLUMN external_url TEXT NULL`, preserve `updated_at` semantics if needed, and ensure any default behavior keeps prior rows unchanged. |
...
| `src/tests/...` *(targeted)* | Add regression coverage for create/update actions (with/without `external_url`) and for UI fallback behavior if feasible (unit/visual tests for library and list surfaces). |
