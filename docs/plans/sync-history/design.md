...
**`GET /api/v1/sync-history/export`** — same filter params + `format` (enum `json|csv`, default `json`). Runs the same filtered `search` (no LIMIT, or a high cap; see risk on memory). Returns a raw `new Response(body, { headers: { 'Content-Type': format === 'csv' ? 'text/csv; charset=utf-8' : 'application/json', 'Content-Disposition': \`attachment; filename="sync-history-${new Date().toISOString()}.${format}"\` } })`— the`api/backups/download/[filename]/+server.ts:28-33`download pattern, but keep`400`/`404`as the`json({ error })`envelope (not SvelteKit`error()`) for v1 consistency. CSV = one row per `sync_history`entry, scalar columns +`changes`/`section_results`as JSON-encoded cells; implement RFC-4180 escaping (quote fields containing`"`,`,`,newline; double embedded quotes) — no repo precedent, write it yourself.
