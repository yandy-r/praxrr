...
1. **Wizard runs only after account setup.** It is layered strictly _after_ the existing `/auth/setup` (admin-account) gate — never before or instead of it.
...
connect/database/profile steps _embed existing forms/components_ or _deep-link into the canonical routes_
is Decision D1.
...
_(Full detail + IDs in `research-security.md`.)_
