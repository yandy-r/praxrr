...
- **SSRF centralization (W1)**: single choke point is `getArrInstanceClient()` in `arrInstanceClients.ts` — confirmed _zero_ call sites for `assertSafeArrUrl()` anywhere in the app (not even partial). Fixing here for free covers `generatePreview()`-based live diff/compare, `arr/library`, `arr/releases`, upgrades.
