...
- Example: `src/lib/server/pcd/core/manager.ts`

**Repository/query modules**: Each persistent entity gets a dedicated query module under `src/lib/server/db/queries/` that exposes a cohesive API for CRUD, lookups, and helpers (e.g., `nameExists`, `getById`, `create`, `update`). Routes, managers, and jobs always consume these exports, which shields consumers from raw SQL and keeps migrations/DB semantics encapsulated.

- Example: `src/lib/server/db/queries/databaseInstances.ts`
...
- Example: `src/routes/databases/new/+page.server.ts`
...
2. Respect the workspace path aliases in `deno.json` when adding modules so downstream packages (app, schema, db) can share code without relative-path fragility; use aggregate entry points like `src/lib/server/pcd/index.ts` when you need to re-export a feature set.
