...
| Configurable default DB auto-link env vars     | `rg -n PRAXRR_DEFAULT_DB_URL packages/praxrr-app/src/hooks.server.ts && rg -n PRAXRR_DEFAULT_DB_BRANCH packages/praxrr-app/src/hooks.server.ts && rg -n PRAXRR_DEFAULT_DB_NAME packages/praxrr-app/src/hooks.server.ts` | Runtime lead (`Task 2.1`)                                              | `packages/praxrr-app/src/hooks.server.ts` references all three vars and handles empty `PRAXRR_DEFAULT_DB_URL` as disabled | [`packages/praxrr-app/src/hooks.server.ts`](../../packages/praxrr-app/src/hooks.server.ts), [`README.md`](../../README.md)                                                                                                                                                       |
...
| Custom PCD URL linking unchanged               | `deno test` (or scoped PCD integration subset including git-url fixtures)                                                                                                                                               | App lead (`Task 2.3`)                                                  | Custom git URL linking paths remain green in integration coverage                                                         | [`packages/praxrr-app/src/lib/server/pcd/git/dependencies.ts`](../../packages/praxrr-app/src/lib/server/pcd/git/dependencies.ts), [`packages/praxrr-app/src/lib/server/pcd/manifest/manifest.ts`](../../packages/praxrr-app/src/lib/server/pcd/manifest/manifest.ts)             |
...
|-- packages/praxrr-app/src/         # SvelteKit app source
...
2. **Regenerate Types**: Run `deno task generate:pcd-types --local`
   - System: Reads local schema, writes updated `packages/praxrr-app/src/lib/shared/pcd/types.ts`
3. **Add DB Ops**: Create/edit SQL files in `packages/praxrr-db/ops/`
   - System: No validation at this step (validated in CI)
4. **Update App Code**: Modify handlers in `packages/praxrr-app/src/lib/server/pcd/`
